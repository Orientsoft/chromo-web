    ...
    easyUploader.prototype = {
        ...
        initPlugin: function (configs) {
            ...
            if (!container) {
                ...
                this.container.before(this.buildUploader());
                this.bindHeadEvent();
            }
        ...
        buildUploader: function() {
            var html = '';
            html+= '<div class="easy-uploader" style="display: none">'
            +  '<div class="btn-box">'
        ...
        bindHeadEvent: function() {
            ...
            this.container.on('click', function () {
                $(".btn-select-file").trigger('click');
            });
            $('#dnd-block').on('dragover', function (e) {
                e.stopPropagation();
                e.preventDefault();
                ...
                that.fileObj = { fileList: [], isReady: true };
                Array.from(files).forEach(function (f) {
                    if (f.type === 'image/jpeg' || f.type === 'image/png') {
                        f.checked = true;
                        that.fileObj.fileList.push(f);
                    }
                });
                that.updateFiles();
                $(this).css('background', '#fff');
            ...
            $('#image-block').on('click', '.delico', function () {
                var el = $(this).parent().parent();
                var index = el.parent().children().index(el);

                that.files.forEach(function(item, i) {
                    if (i === index) {
                       that.files.splice(i, 1);
                       that.configs.onChange && that.configs.onChange(that.fileObj.fileList);
                    }
                });
                el.remove();
                that.updateFilesDom();
...
//                            this.configs.data.append(this.configs.name, this.files[this.ajax.index].file);
                            this.files.forEach(f => this.configs.data.append(this.configs.name, f.file));
                        } else {
                        ...
                        this.ajax.example = $.ajax({
                            ...
                            success: function (res) {
                                if (res[that.configs.successKey] == that.configs.successValue) {
                                    ...
                                    that.files[that.ajax.index].ajaxResponse = res;
                                    that.updateFilesDom();
                                    that.configs.onError && that.configs.onError(res);
                                }
